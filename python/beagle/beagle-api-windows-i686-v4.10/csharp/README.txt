...
Introduction
------------
This directory contains examples that use the Beagle Rosetta .NET
language bindings.

The file beagle.cs is the C# binding that will be compiled and
...
2) Place beagle.dll in the PATH
...
4) Type 'make' at the command line.  This will build the examples and
   also copy beagle.dll to _output.
5) cd _output
6) PROGRAM.exe
...
2) Type 'make' at the command line.  This will build the examples and
   also copy beagle.so to _output/libbeagle.so:  note that the "lib"
   prefix is required for C# interpreter to successfully load the
   software library.
3) cd _output
4) mono PROGRAM

Note that libbeagle.so must be in either the current directory or
